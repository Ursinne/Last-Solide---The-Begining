...
public class WeaponSystem : MonoBehaviour
{
    ...
    private void Update()
    {
        // Uppdatera vapnets position och rotation om det �r utrustat
        if (isEquipped)
        {
            UpdateTransform();
    ...
    private bool PerformRangedAttack()
    {
        ...
        if (projectilePrefab != null)
        {
            ...
            if (rb != null)
            {
                rb.linearVelocity = spawnTransform.forward * projectileSpeed;
            }

            // Om det finns en Projectile-komponent, konfigurera den
            //Projectile projectileScript = projectile.GetComponent<Projectile>();
            //if (projectileScript != null)
            //{
              //  projectileScript.damage = damage;
               // projectileScript.shooter = playerTransform.gameObject;
            //}

            //if (debugMode)
              //  Debug.Log($"Sk�t projektil fr�n {weaponType}. Ammo kvar: {ammoCount}");

            //return true;
        }
        else
        {
            // Raycasting f�r vapnet om ingen projektil finns
            RaycastHit hit;
            if (Physics.Raycast(
                transform.position,
                transform.forward,
                ...
                targetLayers))
            {
                ...
                if (enemy != null)
                {
                    ...
                    if (debugMode)
                        Debug.Log($"Tr�ffade fiende: {hit.collider.name} f�r {damage} skada med raycast");
                ...
                if (zombie != null)
                {
                    ...
                    if (debugMode)
                        Debug.Log($"Tr�ffade zombie: {hit.collider.name} f�r {damage} skada med raycast");
