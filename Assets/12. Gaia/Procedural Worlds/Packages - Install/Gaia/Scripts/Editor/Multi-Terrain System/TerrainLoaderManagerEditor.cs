                ...
                foreach (Vector3 pos in m_terrainLoaderManager.m_freeTerrainSlotsButtonPositions.Keys)
                {
                    ...
                    if (GUI.Button(new Rect(addButtonPos, addButtonSize), "Add"))
                    {
                        ...
                        else
                        {
                            ...
                            float lowestXCorner = 0f - (numberXTiles * 0.5f * tileSize);
                            float lowestZCorner = 0f - (numberXTiles * 0.5f * tileSize);
                            int tx = Mathf.FloorToInt((pos.x - lowestXCorner) / singleTileCreationSettings.m_terrainSize);
                            int tz = Mathf.FloorToInt((pos.z - lowestZCorner) / singleTileCreationSettings.m_terrainSize);
