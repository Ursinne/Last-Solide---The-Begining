...
namespace Gaia
{
    ...
    public class BakedMaskCache : UnityEngine.Object
    {
        ...
        private RenderTexture BakePolyMask(Terrain terrain, PolyMask polyMask)
        {
            PolyMaskVisualization polyMaskVisualization = new PolyMaskVisualization(polyMask, terrain);
            polyMask.RenderPMV(polyMaskVisualization);
            RenderTexture polyTexture = new RenderTexture(polyMask.m_visualizationDestRT);
            Graphics.Blit(polyMask.m_visualizationDestRT, polyTexture);
            DestroyImmediate(polyMaskVisualization.VisObject);
            string fileName = GetPolyMaskFileName(terrain, polyMask);
            polyMask.OnVisualizationUpdated -= OnPolyMaskVisualizationUpdated;
            polyMask.OnVisualizationUpdated += OnPolyMaskVisualizationUpdated;
            polyMask.OnDestroyed -= OnPolyMaskOutOfScope;
            polyMask.OnDestroyed += OnPolyMaskOutOfScope;
            SaveBakedMaskInCache(polyTexture, terrain, fileName);
            return polyTexture;
        }

        private void OnPolyMaskVisualizationUpdated(PolyMask polyMask)
        {
            //On an update of the visualization of a poly mask we need to empty the cache for all instances of that mask so it is re-drawn and cached the next time it
            //is visualized through Gaia
            RemovePolyMaskEntries(polyMask);
           
        }

        private void RemovePolyMaskEntries(PolyMask polyMask)
        {
            string searchString = "_" + polyMask.name + "_";

            for (int i = m_cacheEntries.Length - 1; i >= 0; i--)
            {
                BakedMaskCacheEntry entry = m_cacheEntries[i];
                if (entry.fileName.Contains(searchString))
                {
                    m_cacheEntries = GaiaUtils.RemoveArrayIndexAt(m_cacheEntries, i);
                }
            }
            ...
            var allStampers = Resources.FindObjectsOfTypeAll<Stamper>();
        ...
        public void OnPolyMaskOutOfScope(PolyMask polyMask)
        {
            //When a poly mask goes "out of scope" (is deleted or not selected by an image mask anymore), we remove the poly mask entries from cache and unsubscribe from events
            RemovePolyMaskEntries(polyMask);
            polyMask.OnVisualizationUpdated -= OnPolyMaskVisualizationUpdated;
            polyMask.OnDestroyed -= OnPolyMaskOutOfScope;
        }
        ...
        private string GetPolyMaskFileName(Terrain terrain, PolyMask polyMask)
        {
            return terrain.name + "_" + polyMask.name + "_" + polyMask.ControlPoints.Count();
        }
        ...
        public RenderTexture LoadBakedMask(Terrain terrain, BakedMaskTypeInternal type, CollisionMask collisionMask = null, string worldMaskGUID = "", PolyMask polyMask = null) //float radius, int id = 0, string tag = "", string GUID = "")
        {
            ...
            switch (type)
            {
                ...
                case BakedMaskTypeInternal.LayerTree:
                    ...
                    break;
                case BakedMaskTypeInternal.PolyMask:
                    fileName = GetPolyMaskFileName(terrain, polyMask);
                    break;
                default:
                    return null;
            }
            ...
            if (fileName != "")
            {
                ...
                switch (type)
                {
                    ...
                    case BakedMaskTypeInternal.PolyMask:
                        return BakePolyMask(terrain, polyMask);
                    default:
                        return null;
                }
